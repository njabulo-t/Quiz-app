...
export const QuizHistory = () => {
  ...
  let [question, setQuestion] = useState(dataHistory[index]);
...
const nextQuestion = () => {
  if (lock ===true){
    if(index === dataHistory.length -1) {
      setResult(true);
    } else {

    setIndex(prevIndex => prevIndex + 1);
    setQuestion(dataHistory[index + 1]);
    setLock(false);
  ...
  return (
    <div className='quizContainer container-fluid'>   
      ...
      <div className='end-result'>
        <h2>Your final score: {score} out of {dataHistory.length}</h2>
      {score === dataHistory.length && wrongAnswers.length === 0 &&  (
        <>
      <Confetti />
      ...
      { score !== dataHistory.length && <button onClick={resetQuiz}>Reset</button>}
...
<div className='index'>{index+1} / {dataHistory.length}</div>
